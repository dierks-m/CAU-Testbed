...
try:
    response = request.do_request(server_address, "stop-experiment", request.RequestType.POST, {"id": experiment_id}, api_key)
    print(f'Successfully stopped experiment \'{response["name"]}\'.')
except requests.exceptions.ConnectionError:
    print("Could not connect to server. Perhaps the address is incorrect?")
except RuntimeError as e:
    print("Cannot stop experiment (" + str(e) + ")")
    exit(1)
