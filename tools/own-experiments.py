...
def get_experiment_list(server_address: str, api_key: str,
                        start_delta: timedelta = timedelta(hours=1), end_delta: timedelta = timedelta(hours=12)
                        ):
    ...
    return request.do_request(
        server_address, "list-user-experiments", request.RequestType.GET,
...
def print_experiment_list(experiment_list: list):
    ...
    if tabulate_installed:
        print(tabulate(output_table, headers=["ID", "Name", "Status", "Start", "End"], maxcolwidths=[None, 55],
                       tablefmt='fancy_grid'))
    else:
...
try:
    print_experiment_list(get_experiment_list(server_address, api_key))
